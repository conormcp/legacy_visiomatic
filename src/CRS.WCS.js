...
L.CRS.WCS = L.extend({}, L.CRS, {
    ...
    switch (this.ctype.x.substr(5, 3)) {
      ...
      default:
        ...
        if (!this.options.crval) {
          this.projparam.crval = L.latLng(
            (this.naxis.y + 1.0) / 2.0,
  ...
  celsysToEq: function (latlng) {
    ...
    return L.latLng(
      Math.asin(sd) * invdeg,
      ((Math.atan2(cd2cp * Math.sin(a2), sd2 - sd * cmat[3]) + cmat[0]) *
        invdeg +
        360.0) %
        360.0
    );
  ...
  eqToCelsys: function (latlng) {
    ...
    return L.latLng(
      ...
      ((Math.atan2(cdcp * Math.sin(a), sd2 * cmat[3] - sd) + cmat[1]) * invdeg +
        360.0) %
        360.0
    );
  },
  ...
  rawPixelScale: function (latlng) {
    ...
    if (dlngdx > 180.0) {
      dlngdx -= 360.0;
    } else if (dlngdx < -180.0) {
      dlngdx += 360.0;
    }
    if (dlngdy > 180.0) {
      dlngdy -= 360.0;
    } else if (dlngdy < -180.0) {
      dlngdy += 360.0;
    }
    ...
    return (
      ...
      Math.sqrt(
        Math.abs(
          dlngdx * (latlngdy.lat - latlng.lat) -
  ...
  parseCoords: function (str) {
    ...
    if (typeof latlng === "undefined") {
      ...
      result = /(?:%J\s|^)([-+]?\d+\.?\d*)\s*[,\s]+\s*([-+]?\d+\.?\d*)/g.exec(
        str
      );
  ...
  _celsysmatInit: function (celcode) {
    ...
    switch (celcode) {
      case "galactic":
        ...
        cpole = L.latLng(27.1282512, 192.85948123);
        break;
      case "ecliptic":
      ...
      case "supergalactic":
        ...
        cpole = L.latLng(15.7048, 283.7514);
        break;
      default:
        corig = L.latLng(0.0, 0.0);
    ...
    cmat[1] = Math.asin(
      Math.cos(corig.lat * deg) * Math.sin((cpole.lng - corig.lng) * deg)
    );
